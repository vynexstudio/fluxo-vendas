...
export function LoginPage() {
  ...
  const doLogin = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    const res = await login(f.email, f.pass);
    setBusy(false);
    if (res.err) return setErr({ form: res.err });
    toast.push("success", res.isSuper ? "Bem-vindo ao Painel do Sistema." : "Bem-vindo de volta!");
